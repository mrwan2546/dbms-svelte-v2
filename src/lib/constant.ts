export default {
    ...
    // Address
    getProvinces: "SELECT * FROM address_provinces ORDER BY CONVERT(name_th USING tis620)",
    getDistrictByProvinceId: "SELECT * FROM address_districts WHERE province_id = ? ORDER BY CONVERT(name_th USING tis620)",
    getSubDistrictByDistrictId: "SELECT * FROM address_sub_districts WHERE district_id = ? ORDER BY CONVERT(name_th USING tis620)",
    getAddressSubDistrictById: "SELECT * FROM address_sub_districts WHERE id = ? LIMIT 0,1",
    // Order
    getOrders: "select o.id, o.order_id, s.name as `shipping_name`, s.address as `shipping_address`, CONCAT('ตำบล', asd.name_th, ' อำเภอ', ad.name_th, ' จังหวัด', ap.name_th, ', ', asd.zip_code) as `shipping_province`, s.mobile as `shipping_mobile`,  JSON_ARRAYAGG(JSON_OBJECT('id', p.id, 'units', oi.order_units, 'title', p.title, 'price', oi.order_price, 'image', p.image)) as items from orders o, order_items oi, products p, shipping s, address_sub_districts asd, address_districts ad, address_provinces ap where oi.order_id = o.id AND oi.order_product_id = p.id AND s.order_id = o.id AND asd.id = s.address_id AND asd.district_id = ad.id AND ad.province_id = ap.id AND o.user_id = ?",
